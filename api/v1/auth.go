...
func (s *APIV1Service) SignIn(c echo.Context) error {
	...
	if err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, "找不到系统设置").SetInternal(err)
	}
	if disablePasswordLoginSystemSetting != nil {
		...
		if err != nil {
			return echo.NewHTTPError(http.StatusInternalServerError, "解封系统设置失败").SetInternal(err)
		}
		if disablePasswordLogin {
			return echo.NewHTTPError(http.StatusUnauthorized, "密码登录已停用")
		}
	}

	if err := json.NewDecoder(c.Request().Body).Decode(signin); err != nil {
		return echo.NewHTTPError(http.StatusBadRequest, "登录请求格式错误").SetInternal(err)
	}
	...
	if err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, "登录凭证不正确，请重试")
	}
	if user == nil {
		return echo.NewHTTPError(http.StatusUnauthorized, "登录凭证不正确，请重试")
	} else if user.RowStatus == store.Archived {
		return echo.NewHTTPError(http.StatusForbidden, fmt.Sprintf("用户已存档，用户名为 %s", signin.Username))
	}
...
func (s *APIV1Service) SignUp(c echo.Context) error {
	...
	if err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, "生成密码哈希值失败").SetInternal(err)
	}

	userCreate.PasswordHash = string(passwordHash)
	user, err := s.Store.CreateUser(ctx, userCreate)
	if err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, "创建用户失败").SetInternal(err)
	}
	accessToken, err := auth.GenerateAccessToken(user.Username, user.ID, time.Now().Add(auth.AccessTokenDuration), []byte(s.Secret))
	if err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, fmt.Sprintf("生成tokens失败, err: %s", err)).SetInternal(err)
	}
	if err := s.UpsertAccessTokenToStore(ctx, user, accessToken); err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, fmt.Sprintf("上载用户设置失败, err: %s", err)).SetInternal(err)
	}
	cookieExp := time.Now().Add(auth.CookieExpDuration)
	setTokenCookie(c, auth.AccessTokenCookieName, accessToken, cookieExp)
	userMessage := convertUserFromStore(user)
...
func (s *APIV1Service) UpsertAccessTokenToStore(ctx context.Context, user *store.User, accessToken string) error {
	...
	if err != nil {
		return errors.Wrap(err, "获取用户访问令牌失败")
	}
